...
@static_var(trans_prev=np.zeros(3))
def get_translation(hand, body, alpha=0.8) -> np.ndarray:
	hand_type = hand["type"]
	...
	if CORRECT_Z_AXIS: # This well be set by calibrate
	...
	current_trans = R @ wrist_trans
	if alpha:
		ema_filtered = alpha * current_trans  + (1 - alpha) * get_translation.trans_prev
		get_translation.trans_prev = ema_filtered
		return ema_filtered

	return current_trans

def get_pose(hand, body, ema_alpha=0.1) -> np.ndarray:	
	translation = get_translation(hand, body, ema_alpha)
